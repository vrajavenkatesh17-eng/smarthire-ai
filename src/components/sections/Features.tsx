...
import { Brain, FileSearch, Users, BarChart3, Shield, Zap, MessageSquare, Clock } from "lucide-react";
const features = [{
  icon: Brain,
...
const Features = () => {
  return <section id="features" className="py-24 bg-gradient-subtle overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        duration: 0.6
          ...
          duration: 0.5,
          delay: 0.1
        }} className="text-heading md:text-display-sm mt-6 mb-6 text-[#050505]">
        ...
        margin: "-50px"
          ...
          {features.map((feature, index) => <motion.div key={feature.title} variants={cardVariants} whileHover={{
          y: -8,
          transition: {
            duration: 0.2
          }
            ...
            duration: 0.5
          }} className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${feature.color === "primary" ? "bg-primary/10" : feature.color === "accent" ? "bg-accent/10" : "bg-success/10"}`}>
                <feature.icon className={`w-6 h-6 ${feature.color === "primary" ? "text-primary" : feature.color === "accent" ? "text-accent" : "text-success"}`} />
              </motion.div>
              ...
              {/* Hover indicator */}
              <motion.div className="mt-4 h-0.5 bg-primary rounded-full origin-left" initial={{
            scaleX: 0
          }} whileHover={{
            scaleX: 1
          }} transition={{
